...
export class HermesReelCampaignService {
  ...
  async submit(input: any) {
    ...
    const accounts = await this.prisma.socialAccount.findMany({ where: { status: { in: ['active', 'warming_up'] }, sessionHealth: 'HEALTHY' }, select: { id: true }, orderBy: { username: 'asc' }, take: items.length });
    ...
    for (const { item, accountId, index } of assignments) {
      ...
      await this.queue.add('postJob', { postId: post.id, accountId, campaignId: campaign.id, content: item.caption, mediaUrls: [mediaUrl], spinIndex: index }, { jobId: `hermes-reel:${campaign.id}:${index}`, attempts: 3, backoff: { type: 'exponential', delay: 30000 } });
